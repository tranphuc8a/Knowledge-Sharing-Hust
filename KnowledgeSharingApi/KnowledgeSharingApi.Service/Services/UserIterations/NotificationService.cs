...
namespace KnowledgeSharingApi.Services.Services.UserIterations
{
    ...
    ) : INotificationService
    {
        ...
        /// <summary>
        /// Kiểm tra user tồn tại và trả về ViewUserProfile
        /// </summary>
        /// <param name="userId"> Id của người dùng cần kiểm tra </param>
        /// <returns></returns>
        /// Created: PhucTV (20/3/24)
        /// Modified: None
        protected virtual async Task<ViewUserProfile> CheckUserExisted(Guid userId)
        {
            ViewUserProfile viewUser = await UserRepository.GetDetail(userId)
        ...
        public virtual async Task<ServiceResult> DeleteNotification(Guid userId, Guid notiId)
        {
            ...
            ViewUserProfile user = await CheckUserExisted(userId);
