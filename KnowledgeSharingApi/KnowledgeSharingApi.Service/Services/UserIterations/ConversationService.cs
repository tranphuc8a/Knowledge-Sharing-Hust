...
namespace KnowledgeSharingApi.Services.Services.UserIterations
{
    ...
    ) : IConversationService
    {
        ...
        /// Created: PhucTV (20/3/24)
        /// Modified: None
        protected virtual async Task<ViewUserProfile> CheckExistedUser(Guid userId)
        {
            return (ViewUserProfile)((await UserRepository.GetDetail(userId))?.Clone()
        ...
        public virtual async Task<ServiceResult> GetConversation(Guid myUId, Guid conversationId)
        {
            ...
            //ViewUserProfile user = await CheckExistedUser(myUId);
        ...
        {
            //ViewUserProfile user_1 = await CheckExistedUser(myUid);
        ...
        {
            //ViewUserProfile user_1 = await CheckExistedUser(myUid);
            //ViewUserProfile user_2 = await CheckExistedUser(uid);
        ...
        {
            ViewUserProfile profile = await CheckExistedUser(myUid);
